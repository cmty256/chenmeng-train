...
export default defineComponent({
  ...
  setup() {
    ...
    //   price: "50",
    // }
    // 关于SEAT_TYPE[KEY]：当知道某个具体的属性xxx时，可以用obj.xxx，当属性名是个变量时，可以使用obj[xxx] -- obj["code"]
    const seatTypes = [];
    ...
    // computed和watch类似，都可以用来响应式的计算一个值，computed像这里这样写，直接声明出一个响应式变量
    // 区别：computed是计算属性，watch是监听属性，当监听的属性发生变化时，会触发相应的操作
    ...
    const finishCheckPassenger = () => {
      ...
      // 前端校验不一定准，但前端校验可以减轻后端很多压力，后端校验才是真实的校验
